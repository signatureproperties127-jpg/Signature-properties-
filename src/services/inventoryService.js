...
class InventoryService {
  ...
  create(payload, actor = {}) {
    ...
    const property = {
      ...
      ListingFor:     payload.ListingFor || 'Sale',    // Sale / Rent / Lease
      ListingStatus:  payload.ListingStatus || 'Available',   // Available / Under Offer / Sold / Rented / Off-market
      OwnerName:      payload.OwnerName || null,
      OwnerMobile:    payload.OwnerMobile || null,
      OwnerType:      payload.OwnerType || 'Direct Owner',    // Direct Owner / Builder / Sub-broker
      ...
      Fields:         {},   // catch-all for dynamic V2 fields
      Photos:         [],
      CreatedAt:      now,
      UpdatedAt:      now,
    ...
    // Copy every other field into Fields (dynamic)
    _copyDynamicFields(payload, property);
    db.Inventory.push(property);
    // Ensure per-property upload folder
  ...
  update(propertyId, payload, actor = {}) {
    ...
    for (const [k, v] of Object.entries(payload || {})) {
      ...
      if (['Title','Category','SubCategory','ListingFor','ListingStatus','OwnerName','OwnerMobile','OwnerType','ExclusiveWithMe','SocietyName','BuilderName','ProjectName','Location1','Location2'].includes(k)) {
        p[k] = v;
      } else {
        p.Fields = p.Fields || {};
        p.Fields[k] = v;
